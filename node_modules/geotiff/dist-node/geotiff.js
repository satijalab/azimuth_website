...
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;var e=require("lzw-tiff-decoder"),t=r(require("./basedecoder"));function r(e){return e&&e.__esModule?e:{default:e}}class s extends t.default{constructor(e){super();const t=e.TileWidth||e.ImageWidth,r=e.TileLength||e.ImageLength,s=e.BitsPerSample[0]/8;this.maxUncompressedSize=t*r*s}async decodeBlock(t){const r=new Uint8Array(t);return(await(0,e.decompress)(r,this.maxUncompressedSize)).buffer}}exports.default=s;
...
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;var t=a(require("txml")),e=require("./globals"),r=require("./rgb"),i=require("./compression"),o=require("./resample");function a(t){return t&&t.__esModule?t:{default:t}}function s(t,e,r){let i=0;for(let o=e;o<r;++o)i+=t[o];return i}function n(t,e,r){switch(t){case 1:switch(e){case 8:return new Uint8Array(r);case 16:return new Uint16Array(r);case 32:return new Uint32Array(r)}break;case 2:switch(e){case 8:return new Int8Array(r);case 16:return new Int16Array(r);case 32:return new Int32Array(r)}break;case 3:switch(e){case 32:return new Float32Array(r);case 64:return new Float64Array(r)}}throw Error("Unsupported data format/bitsPerSample")}class l{constructor(t,e,r,i,o,a){this.fileDirectory=t,this.geoKeys=e,this.dataView=r,this.littleEndian=i,this.tiles=o?{}:null,this.isTiled=!t.StripOffsets;const s=t.PlanarConfiguration;if(this.planarConfiguration=void 0===s?1:s,1!==this.planarConfiguration&&2!==this.planarConfiguration)throw new Error("Invalid planar configuration.");this.source=a}getFileDirectory(){return this.fileDirectory}getGeoKeys(){return this.geoKeys}getWidth(){return this.fileDirectory.ImageWidth}getHeight(){return this.fileDirectory.ImageLength}getSamplesPerPixel(){return this.fileDirectory.SamplesPerPixel}getTileWidth(){return this.isTiled?this.fileDirectory.TileWidth:this.getWidth()}getTileHeight(){return this.isTiled?this.fileDirectory.TileLength:void 0!==this.fileDirectory.RowsPerStrip?Math.min(this.fileDirectory.RowsPerStrip,this.getHeight()):this.getHeight()}getBytesPerPixel(){let t=0;for(let e=0;e<this.fileDirectory.BitsPerSample.length;++e){const r=this.fileDirectory.BitsPerSample[e];if(r%8!=0)throw new Error(`Sample bit-width of ${r} is not supported.`);if(r!==this.fileDirectory.BitsPerSample[0])throw new Error("Differing size of samples in a pixel are not supported.");t+=r}return t/8}getSampleByteSize(t){if(t>=this.fileDirectory.BitsPerSample.length)throw new RangeError(`Sample index ${t} is out of range.`);const e=this.fileDirectory.BitsPerSample[t];if(e%8!=0)throw new Error(`Sample bit-width of ${e} is not supported.`);return e/8}getReaderForSample(t){const e=this.fileDirectory.SampleFormat?this.fileDirectory.SampleFormat[t]:1,r=this.fileDirectory.BitsPerSample[t];switch(e){case 1:switch(r){case 8:return DataView.prototype.getUint8;case 16:return DataView.prototype.getUint16;case 32:return DataView.prototype.getUint32}break;case 2:switch(r){case 8:return DataView.prototype.getInt8;case 16:return DataView.prototype.getInt16;case 32:return DataView.prototype.getInt32}break;case 3:switch(r){case 32:return DataView.prototype.getFloat32;case 64:return DataView.prototype.getFloat64}}throw Error("Unsupported data format/bitsPerSample")}getArrayForSample(t,e){return n(this.fileDirectory.SampleFormat?this.fileDirectory.SampleFormat[t]:1,this.fileDirectory.BitsPerSample[t],e)}async getTileOrStrip(t,e,r,i,o){const a=Math.ceil(this.getWidth()/this.getTileWidth()),s=Math.ceil(this.getHeight()/this.getTileHeight());let n;const{tiles:l}=this;let h,c;1===this.planarConfiguration?n=e*a+t:2===this.planarConfiguration&&(n=r*a*s+e*a+t),this.isTiled?(h=this.fileDirectory.TileOffsets[n],c=this.fileDirectory.TileByteCounts[n]):(h=this.fileDirectory.StripOffsets[n],c=this.fileDirectory.StripByteCounts[n]);const p=await this.source.fetch(h,c,!1,o);if(o&&o.aborted)return{x:t,y:e,sample:r,data:[]};let f;return null===l?f=i.decode(this.fileDirectory,p):l[n]||(f=i.decode(this.fileDirectory,p),l[n]=f),{x:t,y:e,sample:r,data:await f}}async _readRaster(t,e,r,i,a,n,l,h,c){const p=this.getTileWidth(),f=this.getTileHeight(),g=Math.max(Math.floor(t[0]/p),0),u=Math.min(Math.ceil(t[2]/p),Math.ceil(this.getWidth()/this.getTileWidth())),m=Math.max(Math.floor(t[1]/f),0),d=Math.min(Math.ceil(t[3]/f),Math.ceil(this.getHeight()/this.getTileHeight())),y=t[2]-t[0];let w=this.getBytesPerPixel();const D=[],M=[];for(let o=0;o<e.length;++o)1===this.planarConfiguration?D.push(s(this.fileDirectory.BitsPerSample,0,e[o])/8):D.push(0),M.push(this.getReaderForSample(e[o]));const S=[],{littleEndian:T}=this;for(let o=m;o<d;++o)for(let s=g;s<u;++s)for(let n=0;n<e.length;++n){const l=n,h=e[n];2===this.planarConfiguration&&(w=this.getSampleByteSize(h));const g=this.getTileOrStrip(s,o,h,a,c);S.push(g),g.then(o=>{if(c&&c.aborted)return;const a=o.data,s=new DataView(a),n=o.y*f,h=o.x*p,g=(o.y+1)*f,u=(o.x+1)*p,m=M[l],d=Math.min(f,f-(g-t[3])),S=Math.min(p,p-(u-t[2]));for(let c=Math.max(0,t[1]-n);c<d;++c)for(let o=Math.max(0,t[0]-h);o<S;++o){const a=(c*p+o)*w,f=m.call(s,a+D[l],T);let g;i?(g=(c+n-t[1])*y*e.length+(o+h-t[0])*e.length+l,r[g]=f):(g=(c+n-t[1])*y+o+h-t[0],r[l][g]=f)}})}if(await Promise.all(S),c&&c.aborted)return new r.constructor(0);if(n&&t[2]-t[0]!==n||l&&t[3]-t[1]!==l){let a;return(a=i?(0,o.resampleInterleaved)(r,t[2]-t[0],t[3]-t[1],n,l,e.length,h):(0,o.resample)(r,t[2]-t[0],t[3]-t[1],n,l,h)).width=n,a.height=l,a}return r.width=n||t[2]-t[0],r.height=l||t[3]-t[1],r}async readRasters({window:t,samples:e=[],interleave:r,pool:o=null,width:a,height:s,resampleMethod:l,fillValue:h,signal:c}={}){const p=t||[0,0,this.getWidth(),this.getHeight()];if(p[0]>p[2]||p[1]>p[3])throw new Error("Invalid subsets");const f=(p[2]-p[0])*(p[3]-p[1]);if(e&&e.length){for(let i=0;i<e.length;++i)if(e[i]>=this.fileDirectory.SamplesPerPixel)return Promise.reject(new RangeError(`Invalid sample index '${e[i]}'.`))}else for(let i=0;i<this.fileDirectory.SamplesPerPixel;++i)e.push(i);let g;if(r){g=n(this.fileDirectory.SampleFormat?Math.max.apply(null,this.fileDirectory.SampleFormat):1,Math.max.apply(null,this.fileDirectory.BitsPerSample),f*e.length),h&&g.fill(h)}else{g=[];for(let t=0;t<e.length;++t){const r=this.getArrayForSample(e[t],f);Array.isArray(h)&&t<h.length?r.fill(h[t]):h&&!Array.isArray(h)&&r.fill(h),g.push(r)}}const u=o||(0,i.getDecoder)(this.fileDirectory);return await this._readRaster(p,e,g,r,u,a,s,l,c)}async readRGB({window:t,pool:i=null,width:o,height:a,resampleMethod:s,enableAlpha:n=!1,signal:l}={}){const h=t||[0,0,this.getWidth(),this.getHeight()];if(h[0]>h[2]||h[1]>h[3])throw new Error("Invalid subsets");const c=this.fileDirectory.PhotometricInterpretation;if(c===e.photometricInterpretations.RGB){let r=[0,1,2];if(this.fileDirectory.ExtraSamples!==e.ExtraSamplesValues.Unspecified&&n){r=[];for(let t=0;t<this.fileDirectory.BitsPerSample.length;t+=1)r.push(t)}return this.readRasters({window:t,interleave:!0,samples:r,pool:i,width:o,height:a,signal:l})}let p;switch(c){case e.photometricInterpretations.WhiteIsZero:case e.photometricInterpretations.BlackIsZero:case e.photometricInterpretations.Palette:p=[0];break;case e.photometricInterpretations.CMYK:p=[0,1,2,3];break;case e.photometricInterpretations.YCbCr:case e.photometricInterpretations.CIELab:p=[0,1,2];break;default:throw new Error("Invalid or unsupported photometric interpretation.")}const f={window:h,interleave:!0,samples:p,pool:i,width:o,height:a,resampleMethod:s,signal:l},{fileDirectory:g}=this,u=await this.readRasters(f),m=2**this.fileDirectory.BitsPerSample[0];let d;switch(c){case e.photometricInterpretations.WhiteIsZero:d=(0,r.fromWhiteIsZero)(u,m);break;case e.photometricInterpretations.BlackIsZero:d=(0,r.fromBlackIsZero)(u,m);break;case e.photometricInterpretations.Palette:d=(0,r.fromPalette)(u,g.ColorMap);break;case e.photometricInterpretations.CMYK:d=(0,r.fromCMYK)(u);break;case e.photometricInterpretations.YCbCr:d=(0,r.fromYCbCr)(u);break;case e.photometricInterpretations.CIELab:d=(0,r.fromCIELab)(u);break;default:throw new Error("Unsupported photometric interpretation.")}return d.width=u.width,d.height=u.height,d}getTiePoints(){if(!this.fileDirectory.ModelTiepoint)return[];const t=[];for(let e=0;e<this.fileDirectory.ModelTiepoint.length;e+=6)t.push({i:this.fileDirectory.ModelTiepoint[e],j:this.fileDirectory.ModelTiepoint[e+1],k:this.fileDirectory.ModelTiepoint[e+2],x:this.fileDirectory.ModelTiepoint[e+3],y:this.fileDirectory.ModelTiepoint[e+4],z:this.fileDirectory.ModelTiepoint[e+5]});return t}getGDALMetadata(e=null){const r={};if(!this.fileDirectory.GDAL_METADATA)return null;const i=this.fileDirectory.GDAL_METADATA,o=(0,t.default)(i.substring(0,i.length-1));if(!o[0].tagName)throw new Error("Failed to parse GDAL metadata XML.");const a=o[0];if("GDALMetadata"!==a.tagName)throw new Error("Unexpected GDAL metadata XML tag.");let s=a.children.filter(t=>"Item"===t.tagName);null!==e&&(s=s.filter(t=>Number(t.attributes.sample)===e));for(let t=0;t<s.length;++t){const e=s[t];r[e.attributes.name]=e.children[0]}return r}getGDALNoData(){if(!this.fileDirectory.GDAL_NODATA)return null;const t=this.fileDirectory.GDAL_NODATA;return Number(t.substring(0,t.length-1))}getOrigin(){const t=this.fileDirectory.ModelTiepoint,e=this.fileDirectory.ModelTransformation;if(t&&6===t.length)return[t[3],t[4],t[5]];if(e)return[e[3],e[7],e[11]];throw new Error("The image does not have an affine transformation.")}getResolution(t=null){const e=this.fileDirectory.ModelPixelScale,r=this.fileDirectory.ModelTransformation;if(e)return[e[0],-e[1],e[2]];if(r)return[r[0],r[5],r[10]];if(t){const[e,r,i]=t.getResolution();return[e*t.getWidth()/this.getWidth(),r*t.getHeight()/this.getHeight(),i*t.getWidth()/this.getWidth()]}throw new Error("The image does not have an affine transformation.")}pixelIsArea(){return 1===this.geoKeys.GTRasterTypeGeoKey}getBoundingBox(){const t=this.getOrigin(),e=this.getResolution(),r=t[0],i=t[1],o=r+e[0]*this.getWidth(),a=i+e[1]*this.getHeight();return[Math.min(r,o),Math.min(i,a),Math.max(r,o),Math.max(i,a)]}}var h=l;exports.default=h;
...
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.default=void 0;var e=require("threads");const r="undefined"!=typeof navigator?navigator.hardwareConcurrency:null;class t{constructor(t=r,o=new e.Worker("/decoder.worker.2d15975d.js")){this.pool=(0,e.Pool)(()=>(0,e.spawn)(o),t)}async decode(r,t){return new Promise((o,a)=>{this.pool.queue(async s=>{try{const c=await s(r,(0,e.Transfer)(t));o(c)}catch(n){a(n)}})})}destroy(){this.pool.terminate(!0)}}var o=t;exports.default=o;
},{"./decoder.worker.js":[["decoder.worker.2d15975d.js","LHx9"],"decoder.worker.2d15975d.js.map","LHx9"]}],"cUx7":[function(require,module,exports) {
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.makeFetchSource=u,exports.makeXHRSource=h,exports.makeHttpSource=b,exports.makeRemoteSource=d,exports.makeBufferSource=g,exports.makeFileSource=y,exports.makeFileReaderSource=m;var e=require("buffer"),t=require("fs"),s=n(require("http")),o=n(require("https")),r=n(require("url"));function n(e){return e&&e.__esModule?e:{default:e}}function a(e){if(!e.every(Boolean))return;const t=new AbortController;function s(){if(t.count-=1,0===t.count){t.abort();for(const t of e)t.removeEventListener("abort",s)}}t.count=e.length;for(const o of e)o.aborted?s():o.addEventListener("abort",s);return t.signal}function i(e,t,s){const o=t+s,r=new ArrayBuffer(s),n=new Uint8Array(r);for(const a of e){const e=a.offset-t,s=a.top-o;let r,i=0,c=0;e<0?i=-e:e>0&&(c=e),r=s<0?a.length-i:o-a.offset-i;const l=new Uint8Array(a.data,i,r);n.set(l,c)}return r}function c(e){if(0===e.length)return[];const t=[];let s=[];t.push(s);for(let o=0;o<e.length;++o)0===o||e[o]===e[o-1]+1?s.push(e[o]):(s=[e[o]],t.push(s));return t}async function l(e){return new Promise(t=>setTimeout(t,e))}class f{constructor(e,{blockSize:t=65536,getFileSize:s=(async()=>null)}={}){this.retrievalFunction=e,this.blockSize=t,this.blockRequests=new Map,this.blocks=new Map,this.signals=new Map,this.abortedBlockIds=new Set,this.blockIdsAwaitingRequest=null,this.fileSizePromise=s()}async fetch(e,t,s=!1,o){const r=await this.fileSizePromise,n=r?Math.min(e+t,r):e+t,f=[],u=[],h=[];for(let a=Math.floor(e/this.blockSize)*this.blockSize;a<n;a+=this.blockSize){const e=Math.floor(a/this.blockSize);this.blocks.has(e)||this.blockRequests.has(e)||(u.push(e),this.signals.has(e)?this.signals.get(e).push(o):this.signals.set(e,[o])),this.blockRequests.has(e)&&h.push(this.blockRequests.get(e)),f.push(e)}if(this.blockIdsAwaitingRequest)for(let a=0;a<u.length;++a)this.blockIdsAwaitingRequest.add(u[a]);else this.blockIdsAwaitingRequest=new Set(u);if(s||await l(),this.blockIdsAwaitingRequest){const e=c(Array.from(this.blockIdsAwaitingRequest).sort());for(const t of e){const e=Array.from(new Set(t.map(e=>this.signals.get(e)).flat())),s=this.requestData(t[0]*this.blockSize,t.length*this.blockSize,a(e));for(let o=0;o<t.length;++o){const e=t[o];this.blockRequests.set(e,(async()=>{try{const r=await s,n=o*this.blockSize,a=Math.min(n+this.blockSize,r.data.byteLength),i=r.data.slice(n,a);this.blockRequests.delete(e),this.signals.delete(e),this.abortedBlockIds.delete(e),this.blocks.set(e,{data:i,offset:r.offset+n,length:i.byteLength,top:r.offset+a})}catch(t){if("AbortError"!==t.name)throw t;this.blocks.delete(e),this.blockRequests.delete(e),this.signals.delete(e),this.abortedBlockIds.add(e)}})())}}this.blockIdsAwaitingRequest=null}const b=[];for(const a of u)this.blockRequests.has(a)&&b.push(this.blockRequests.get(a));await Promise.all(b),await Promise.all(h);const d=f.map(e=>this.blocks.get(e)),g=[];if(d.some(e=>!e)){if(!o||o.aborted)return[];return f.forEach(e=>{if(this.abortedBlockIds.has(e)){const t=this.requestData(e*this.blockSize,this.blockSize);this.blockRequests.set(e,(async()=>{const s=await t,o=Math.min(this.blockSize,s.data.byteLength),r=s.data.slice(0,o);this.abortedBlockIds.delete(e),this.blocks.set(e,{data:r,offset:s.offset,length:r.byteLength,top:s.offset+o})})()),g.push(this.blockRequests.get(e))}}),await Promise.all(g),i(f.map(e=>this.blocks.get(e)),e,t)}return i(d,e,t)}async requestData(e,t,s){const o=await this.retrievalFunction(e,t,s);return o.length?o.length!==o.data.byteLength&&(o.data=o.data.slice(0,o.length)):o.length=o.data.byteLength,o.top=o.offset+o.length,o}}function u(e,{headers:t={},blockSize:s}={}){return new f(async(s,o,r)=>{const n=await fetch(e,{headers:{...t,Range:`bytes=${s}-${s+o-1}`},signal:r});if(n.ok){if(206===n.status){return{data:n.arrayBuffer?await n.arrayBuffer():(await n.buffer()).buffer,offset:s,length:o}}{const e=n.arrayBuffer?await n.arrayBuffer():(await n.buffer()).buffer;return{data:e,offset:0,length:e.byteLength}}}throw new Error("Error fetching data.")},{blockSize:s,getFileSize:async()=>{const s=await fetch(e,{headers:{...t,Range:"bytes=0-1"}});if(!s.ok)throw new Error("Error fetching data.");if(206===s.status){const e=s.headers.get("content-range");if(!e)return null;const[t]=e.split("/").slice(-1);return Number(t)}}})}function h(e,{headers:t={},blockSize:s}={}){return new f(async(s,o)=>new Promise((r,n)=>{const a=new XMLHttpRequest;a.open("GET",e),a.responseType="arraybuffer";const i={...t,Range:`bytes=${s}-${s+o-1}`};for(const[e,t]of Object.entries(i))a.setRequestHeader(e,t);a.onload=(()=>{const e=a.response;206===a.status?r({data:e,offset:s,length:o}):r({data:e,offset:0,length:e.byteLength})}),a.onerror=n,a.send()}),{blockSize:s})}function b(t,{headers:n={},blockSize:a}={}){return new f(async(a,i)=>new Promise((c,l)=>{const f=r.default.parse(t);("http:"===f.protocol?s.default:o.default).get({...f,headers:{...n,Range:`bytes=${a}-${a+i-1}`}},t=>{const s=[];t.on("data",e=>{s.push(e)}),t.on("end",()=>{const t=e.Buffer.concat(s).buffer;c({data:t,offset:a,length:t.byteLength})})}).on("error",l)}),{blockSize:a})}function d(e,t){const{forceXHR:o}=t;if("function"==typeof fetch&&!o)return u(e,t);if("undefined"!=typeof XMLHttpRequest)return h(e,t);if(s.default.get)return b(e,t);throw new Error("No remote source available")}function g(e){return{fetch:async(t,s)=>e.slice(t,t+s)}}function k(e){return new Promise((s,o)=>{(0,t.close)(e,e=>{e?o(e):s()})})}function w(e,s,o){return new Promise((r,n)=>{(0,t.open)(e,s,o,(e,t)=>{e?n(e):r(t)})})}function p(...e){return new Promise((s,o)=>{(0,t.read)(...e,(e,t,r)=>{e?o(e):s({bytesRead:t,buffer:r})})})}function y(t){const s=w(t,"r");return{async fetch(t,o){const r=await s,{buffer:n}=await p(r,e.Buffer.alloc(o),0,o,t);return n.buffer},async close(){const e=await s;return await k(e)}}}function m(e){return{fetch:async(t,s)=>new Promise((o,r)=>{const n=e.slice(t,t+s),a=new FileReader;a.onload=(e=>o(e.target.result)),a.onerror=r,a.readAsArrayBuffer(n)})}}
...
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.setLogger=t,exports.log=n,exports.info=o,exports.warn=i,exports.error=u,exports.time=s,exports.timeEnd=f;class e{log(){}info(){}warn(){}error(){}time(){}timeEnd(){}}let r=new e;function t(t=new e){r=t}function n(...e){return r.log(...e)}function o(...e){return r.info(...e)}function i(...e){return r.warn(...e)}function u(...e){return r.error(...e)}function s(...e){return r.time(...e)}function f(...e){return r.timeEnd(...e)}
},{}],"bsJs":[function(require,module,exports) {
"use strict";Object.defineProperty(exports,"__esModule",{value:!0}),exports.fromUrl=b,exports.fromArrayBuffer=A,exports.fromFile=D,exports.fromBlob=O,exports.fromUrls=S,exports.writeArrayBuffer=x,Object.defineProperty(exports,"Pool",{enumerable:!0,get:function(){return r.default}}),Object.defineProperty(exports,"getDecoder",{enumerable:!0,get:function(){return l.getDecoder}}),Object.defineProperty(exports,"setLogger",{enumerable:!0,get:function(){return f.setLogger}}),exports.rgb=exports.globals=exports.MultiGeoTIFF=exports.default=exports.GeoTIFF=void 0;var e=d(require("./geotiffimage")),t=d(require("./dataview64")),i=d(require("./dataslice")),r=d(require("./pool")),s=require("./source"),n=u(require("./globals"));exports.globals=n;var a=require("./geotiffwriter"),o=u(require("./rgb"));exports.rgb=o;var l=require("./compression"),f=require("./logging");function c(){if("function"!=typeof WeakMap)return null;var e=new WeakMap;return c=function(){return e},e}function u(e){if(e&&e.__esModule)return e;if(null===e||"object"!=typeof e&&"function"!=typeof e)return{default:e};var t=c();if(t&&t.has(e))return t.get(e);var i={},r=Object.defineProperty&&Object.getOwnPropertyDescriptor;for(var s in e)if(Object.prototype.hasOwnProperty.call(e,s)){var n=r?Object.getOwnPropertyDescriptor(e,s):null;n&&(n.get||n.set)?Object.defineProperty(i,s,n):i[s]=e[s]}return i.default=e,t&&t.set(e,i),i}function d(e){return e&&e.__esModule?e:{default:e}}function h(e){switch(e){case n.fieldTypes.BYTE:case n.fieldTypes.ASCII:case n.fieldTypes.SBYTE:case n.fieldTypes.UNDEFINED:return 1;case n.fieldTypes.SHORT:case n.fieldTypes.SSHORT:return 2;case n.fieldTypes.LONG:case n.fieldTypes.SLONG:case n.fieldTypes.FLOAT:case n.fieldTypes.IFD:return 4;case n.fieldTypes.RATIONAL:case n.fieldTypes.SRATIONAL:case n.fieldTypes.DOUBLE:case n.fieldTypes.LONG8:case n.fieldTypes.SLONG8:case n.fieldTypes.IFD8:return 8;default:throw new RangeError(`Invalid field type: ${e}`)}}function g(e){const t=e.GeoKeyDirectory;if(!t)return null;const i={};for(let r=4;r<=4*t[3];r+=4){const s=n.geoKeyNames[t[r]],a=t[r+1]?n.fieldTagNames[t[r+1]]:null,o=t[r+2],l=t[r+3];let f=null;if(a){if(null==(f=e[a]))throw new Error(`Could not get value of geoKey '${s}'.`);"string"==typeof f?f=f.substring(l,l+o-1):f.subarray&&(f=f.subarray(l,l+o),1===o&&(f=f[0]))}else f=l;i[s]=f}return i}function y(e,t,i,r){let s=null,a=null;const o=h(t);switch(t){case n.fieldTypes.BYTE:case n.fieldTypes.ASCII:case n.fieldTypes.UNDEFINED:s=new Uint8Array(i),a=e.readUint8;break;case n.fieldTypes.SBYTE:s=new Int8Array(i),a=e.readInt8;break;case n.fieldTypes.SHORT:s=new Uint16Array(i),a=e.readUint16;break;case n.fieldTypes.SSHORT:s=new Int16Array(i),a=e.readInt16;break;case n.fieldTypes.LONG:case n.fieldTypes.IFD:s=new Uint32Array(i),a=e.readUint32;break;case n.fieldTypes.SLONG:s=new Int32Array(i),a=e.readInt32;break;case n.fieldTypes.LONG8:case n.fieldTypes.IFD8:s=new Array(i),a=e.readUint64;break;case n.fieldTypes.SLONG8:s=new Array(i),a=e.readInt64;break;case n.fieldTypes.RATIONAL:s=new Uint32Array(2*i),a=e.readUint32;break;case n.fieldTypes.SRATIONAL:s=new Int32Array(2*i),a=e.readInt32;break;case n.fieldTypes.FLOAT:s=new Float32Array(i),a=e.readFloat32;break;case n.fieldTypes.DOUBLE:s=new Float64Array(i),a=e.readFloat64;break;default:throw new RangeError(`Invalid field type: ${t}`)}if(t!==n.fieldTypes.RATIONAL&&t!==n.fieldTypes.SRATIONAL)for(let n=0;n<i;++n)s[n]=a.call(e,r+n*o);else for(let n=0;n<i;n+=2)s[n]=a.call(e,r+n*o),s[n+1]=a.call(e,r+(n*o+4));return t===n.fieldTypes.ASCII?new TextDecoder("utf-8").decode(s):s}class p{constructor(e,t,i){this.fileDirectory=e,this.geoKeyDirectory=t,this.nextIFDByteOffset=i}}class w extends Error{constructor(e){super(`No image at index ${e}`),this.index=e}}class T{async readRasters(e={}){const{window:t,width:i,height:r}=e;let{resX:s,resY:n,bbox:a}=e;const o=await this.getImage();let l=o;const f=await this.getImageCount(),c=o.getBoundingBox();if(t&&a)throw new Error('Both "bbox" and "window" passed.');if(i||r){if(t){const[e,i]=o.getOrigin(),[r,s]=o.getResolution();a=[e+t[0]*r,i+t[1]*s,e+t[2]*r,i+t[3]*s]}const e=a||c;if(i){if(s)throw new Error("Both width and resX passed");s=(e[2]-e[0])/i}if(r){if(n)throw new Error("Both width and resY passed");n=(e[3]-e[1])/r}}if(s||n){const e=[];for(let t=0;t<f;++t){const i=await this.getImage(t),{SubfileType:r,NewSubfileType:s}=i.fileDirectory;(0===t||2===r||1&s)&&e.push(i)}e.sort((e,t)=>e.getWidth()-t.getWidth());for(let t=0;t<e.length;++t){const i=e[t],r=(c[2]-c[0])/i.getWidth(),a=(c[3]-c[1])/i.getHeight();if(l=i,s&&s>r||n&&n>a)break}}let u=t;if(a){const[e,t]=o.getOrigin(),[i,r]=l.getResolution(o);u=[Math.round((a[0]-e)/i),Math.round((a[1]-t)/r),Math.round((a[2]-e)/i),Math.round((a[3]-t)/r)],u=[Math.min(u[0],u[2]),Math.min(u[1],u[3]),Math.max(u[0],u[2]),Math.max(u[1],u[3])]}return l.readRasters({...e,window:u})}}class m extends T{constructor(e,t,i,r,s={}){super(),this.source=e,this.littleEndian=t,this.bigTiff=i,this.firstIFDOffset=r,this.cache=s.cache||!1,this.ifdRequests=[],this.ghostValues=null}async getSlice(e,t){const r=this.bigTiff?4048:1024;return new i.default(await this.source.fetch(e,void 0!==t?t:r),e,this.littleEndian,this.bigTiff)}async parseFileDirectoryAt(e){const t=this.bigTiff?20:12,i=this.bigTiff?8:2;let r=await this.getSlice(e);const s=this.bigTiff?r.readUint64(e):r.readUint16(e),a=s*t+(this.bigTiff?16:6);r.covers(e,a)||(r=await this.getSlice(e,a));const o={};let l=e+(this.bigTiff?8:2);for(let u=0;u<s;l+=t,++u){const e=r.readUint16(l),t=r.readUint16(l+2),i=this.bigTiff?r.readUint64(l+4):r.readUint32(l+4);let s,a;const f=h(t),c=l+(this.bigTiff?12:8);if(f*i<=(this.bigTiff?8:4))s=y(r,t,i,c);else{const e=r.readOffset(c),n=h(t)*i;if(r.covers(e,n))s=y(r,t,i,e);else{s=y(await this.getSlice(e,n),t,i,e)}}a=1===i&&-1===n.arrayFields.indexOf(e)&&t!==n.fieldTypes.RATIONAL&&t!==n.fieldTypes.SRATIONAL?s[0]:s,o[n.fieldTagNames[e]]=a}const f=g(o),c=r.readOffset(e+i+t*s);return new p(o,f,c)}async requestIFD(e){if(this.ifdRequests[e])return this.ifdRequests[e];if(0===e)return this.ifdRequests[e]=this.parseFileDirectoryAt(this.firstIFDOffset),this.ifdRequests[e];if(!this.ifdRequests[e-1])try{this.ifdRequests[e-1]=this.requestIFD(e-1)}catch(t){if(t instanceof w)throw new w(e);throw t}return this.ifdRequests[e]=(async()=>{const t=await this.ifdRequests[e-1];if(0===t.nextIFDByteOffset)throw new w(e);return this.parseFileDirectoryAt(t.nextIFDByteOffset)})(),this.ifdRequests[e]}async getImage(t=0){const i=await this.requestIFD(t);return new e.default(i.fileDirectory,i.geoKeyDirectory,this.dataView,this.littleEndian,this.cache,this.source)}async getImageCount(){let e=0,t=!0;for(;t;)try{await this.requestIFD(e),++e}catch(i){if(!(i instanceof w))throw i;t=!1}return e}async getGhostValues(){const e=this.bigTiff?16:8;if(this.ghostValues)return this.ghostValues;const t="GDAL_STRUCTURAL_METADATA_SIZE=",i=t.length+100;let r=await this.getSlice(e,i);if(t===y(r,n.fieldTypes.ASCII,t.length,e)){const t=y(r,n.fieldTypes.ASCII,i,e).split("\n")[0],s=Number(t.split("=")[1].split(" ")[0])+t.length;s>i&&(r=await this.getSlice(e,s));const a=y(r,n.fieldTypes.ASCII,s,e);this.ghostValues={},a.split("\n").filter(e=>e.length>0).map(e=>e.split("=")).forEach(([e,t])=>{this.ghostValues[e]=t})}return this.ghostValues}static async fromSource(e,i){const r=await e.fetch(0,1024),s=new t.default(r),n=s.getUint16(0,0);let a;if(18761===n)a=!0;else{if(19789!==n)throw new TypeError("Invalid byte order value.");a=!1}const o=s.getUint16(2,a);let l;if(42===o)l=!1;else{if(43!==o)throw new TypeError("Invalid magic number.");if(l=!0,8!==s.getUint16(4,a))throw new Error("Unsupported offset byte-size.")}const f=l?s.getUint64(8,a):s.getUint32(4,a);return new m(e,a,l,f,i)}close(){return"function"==typeof this.source.close&&this.source.close()}}exports.GeoTIFF=m;var I=m;exports.default=I;class F extends T{constructor(e,t){super(),this.mainFile=e,this.overviewFiles=t,this.imageFiles=[e].concat(t),this.fileDirectoriesPerFile=null,this.fileDirectoriesPerFileParsing=null,this.imageCount=null}async parseFileDirectoriesPerFile(){const e=[this.mainFile.parseFileDirectoryAt(this.mainFile.firstIFDOffset)].concat(this.overviewFiles.map(e=>e.parseFileDirectoryAt(e.firstIFDOffset)));return this.fileDirectoriesPerFile=await Promise.all(e),this.fileDirectoriesPerFile}async getImage(t=0){await this.getImageCount(),await this.parseFileDirectoriesPerFile();let i=0,r=0;for(let s=0;s<this.imageFiles.length;s++){const n=this.imageFiles[s];for(let a=0;a<this.imageCounts[s];a++){if(t===i){const t=await n.requestIFD(r);return new e.default(t.fileDirectory,n.geoKeyDirectory,n.dataView,n.littleEndian,n.cache,n.source)}i++,r++}r=0}throw new RangeError("Invalid image index")}async getImageCount(){if(null!==this.imageCount)return this.imageCount;const e=[this.mainFile.getImageCount()].concat(this.overviewFiles.map(e=>e.getImageCount()));return this.imageCounts=await Promise.all(e),this.imageCount=this.imageCounts.reduce((e,t)=>e+t,0),this.imageCount}}async function b(e,t={}){return m.fromSource((0,s.makeRemoteSource)(e,t))}async function A(e){return m.fromSource((0,s.makeBufferSource)(e))}async function D(e){return m.fromSource((0,s.makeFileSource)(e))}async function O(e){return m.fromSource((0,s.makeFileReaderSource)(e))}async function S(e,t=[],i={}){const r=await m.fromSource((0,s.makeRemoteSource)(e,i)),n=await Promise.all(t.map(e=>m.fromSource((0,s.makeRemoteSource)(e,i))));return new F(r,n)}async function x(e,t){return(0,a.writeGeotiff)(e,t)}exports.MultiGeoTIFF=F;
},{"./geotiffimage":"eOWo","./dataview64":"dqpX","./dataslice":"dGLV","./pool":"dHPO","./source":"cUx7","./globals":"j27V","./geotiffwriter":"BGyE","./rgb":"fpBl","./compression":"FGCZ","./logging":"dy4f"}]},{},["bsJs"], null)
